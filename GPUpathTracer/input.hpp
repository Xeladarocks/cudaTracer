...
void keyboardfunct(GLFWwindow* window, int key, int scancode, int action, int mods, Camera &camera) {
	...
	switch(key){
		...
		case GLFW_KEY_C: render_target_settings(); break;
	}
	if (key != GLFW_KEY_C) {
		updateMeasuredValue(); // update that camera is moving
	}
}
...
void updateCamera(Camera &camera) {
	if (camera.controls.w) {
		glm::vec3 direction = camera.rotation.yawMat * glm::vec3(0, 0, 1);
	...
	if (camera.controls.a) {
		glm::vec3 direction = camera.rotation.yawMat * glm::vec3(1, 0, 0);
